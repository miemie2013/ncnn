...
from my_tests.mmdet_nets import CoordConv
from my_tests.mmdet_trans import ResizeImage, NormalizeImage, Permute
from my_tests.mmdet_pre import PPYOLOEValTransform


model = CoordConv(3, 3, 3, 1)
model.eval()
torch.save(model.state_dict(), "10.pth")

bp = open('10_pncnn.bin', 'wb')
pp = ''
layer_id = 0
tensor_id = 0
...
with open('10_pncnn.param', 'w', encoding='utf-8') as f:
    f.write(pp)
    f.close()
