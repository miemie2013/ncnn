...
from my_tests.mmdet_nets import EffectiveSELayer



model = EffectiveSELayer(3, act='hardsigmoid', act_name='hardsigmoid')
model.eval()
torch.save(model.state_dict(), "02.pth")

bp = open('02_pncnn.bin', 'wb')
pp = ''
layer_id = 0
tensor_id = 0
pp += 'Input\tlayer_%.8d\t0 1 tensor_%.8d 0=4 1=4 2=3\n' % (layer_id, tensor_id)
layer_id += 1
tensor_id += 1
...
with open('02_pncnn.param', 'w', encoding='utf-8') as f:
    f.write(pp)
    f.close()
